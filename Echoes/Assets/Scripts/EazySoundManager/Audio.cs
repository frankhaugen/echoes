    ...
    public class Audio
    {
        ...
        public Transform SourceTransform
        {
            ...
            set
            {
                if(value == null)
                {
                    sourceTransform = EazySoundManager.EazySoundManager.Gameobject.transform;
                }
                else
                {
        ...
        public Audio(AudioType audioType, AudioClip clip, bool loop, bool persist, float volume, float fadeInValue, float fadeOutValue, Transform sourceTransform)
        {
            ...
            Pitch = 1;
            StereoPan = 0;
            if (sourceTransform != null && sourceTransform != EazySoundManager.EazySoundManager.Gameobject.transform)
            {
                SpatialBlend = 1;
            }
            ReverbZoneMix = 1;
        ...
        public void Play(float volume)
        {
            ...
            if (Pooled)
            {
                ...
                bool restoredFromPool = EazySoundManager.EazySoundManager.RestoreAudioFromPool(Type, AudioID);
                if (!restoredFromPool)
                {
                    return;
                }
        ...
        public void Update()
        {
            ...
            switch (Type)
            {
                case AudioType.Music:
                    {
                        AudioSource.volume = Volume * EazySoundManager.EazySoundManager.GlobalMusicVolume * EazySoundManager.EazySoundManager.GlobalVolume;
                        break;
                    }
                case AudioType.Sound:
                    {
                        AudioSource.volume = Volume * EazySoundManager.EazySoundManager.GlobalSoundsVolume * EazySoundManager.EazySoundManager.GlobalVolume;
                        break;
                    }
                case AudioType.UISound:
                    {
                        AudioSource.volume = Volume * EazySoundManager.EazySoundManager.GlobalUISoundsVolume * EazySoundManager.EazySoundManager.GlobalVolume;
                        break;
                    }
            }
